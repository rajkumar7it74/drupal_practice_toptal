 ...
 *
 * @FieldWidget(
 *   id = "link_fragment_dynamic_select",
 *   label = @Translation("Link Fragment Dynamic Select Widget"),
 *   field_types = {"link"}
 * )
 */
class LinkFragmentDynamicSelectWidget extends LinkWidget {
    ...
    public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
        ...
        $default_selected = $stored['fragmentwidget']['selected'] ?? '';

        if ($default_selected === '' && is_string($uri) && str_contains($uri, '#')) {
            $default_selected = explode('#', $uri, 2)[1] ?? '';
        }

        // Add our new select element INSIDE the same widget.
        $element['dynamic_fragment_select'] = [
            '#type' => 'select',
            '#title' => $this->t('Fragments on the selected page'),
            '#options' => $options,
    ...
    public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
        ...
        $selected = $value['dynamic_fragment_select'] ?? '';

        // Persist it inside link options.
        $value['options']['fragmentwidget']['selected'] = $selected;

        // Append fragment into URI.
        if (!empty($selected) && !empty($value['uri']) && is_string($value['uri'])) {
            $value['uri'] = $this->AddOrReplaceFragment($value['uri'], $selected);
        }

        // Remove the extra key so Field API doesn’t complain.
        unset($value['dynamic_fragment_select']);
        }
        return $values;
    }
    ...
    protected function buildSelectOptions(string $uri): array {
        ...
        $options = [];
        // Code to set id to section paragraph to implement url fragment.
        if ($node && $node->hasField('field_ho_page_content')) {
            foreach ($node->get('field_ho_page_content')->referencedEntities() as $paragraph) {
                ...
                if ($paragraph->hasField('field_ho_section_header') && !$paragraph->get('field_ho_section_header')->isEmpty()) {
                    // Strip HTML tags.  
                    $clean_value = strip_tags($paragraph->field_ho_section_header->value);
                    // Split into words.
                    $words = preg_split('/\s+/', trim($clean_value));
                    ...
                    $fragment_id = substr($label, 0, 30);
                    //$variables['attributes']['id'] = $fragment_id;
                }
    ...
    /**
     * Add or replace #fragment in a Drupal link URI string.
     * Supports: entity:node/123, internal:/path, /path
     */
    protected function AddOrReplaceFragment(string $uri, string $fragment): string {
